...
module.exports = function () {
  var year_month = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
  ...
  d3.csv(dated).then(function (data) {
    ...
    if (char_generated) {
      svg.selectAll("mybar").data(data).enter();
      d3.selectAll('rect').transition().attr("x", function (d) {
        return x(d.Disease);
      }).attr("y", function (d) {
        return y(d.Count);
      ...
      svg.selectAll("mybar").data(data).enter().append("rect").attr("disease", function (d) {
        ...
        d3.select(this).transition().duration(50).attr("opacity", .85); // show hover window

        info.transition().duration(50).attr("opacity", 1); // set the content and position the hover window

        info.html("<strong>".concat(d.Disease, "</strong><br>\n                       <strong>Date:</strong> ").concat(document.getElementById("month-year").innerText, "<br>\n                       <strong>Infected Number:</strong> ").concat(parseInt(d.Count))).style("left", d3.event.pageX + 10 + "px").style("top", d3.event.pageY - 15 + "px").style("position", "absolute");
      }).on('mouseout', function (d, i) {
        d3.select(this).transition().duration(50).attr("opacity", 1);
        info.transition().duration(50).attr("opacity", 0);
      });
    }
    ...
    if (!char_generated) {
      lsvg.selectAll("mydots").data(keys).enter().append("circle").attr("class", "legend-circles").attr("cx", 60).attr("cy", function (d, i) {
        return 100 + i * 40;
    ...
    if (!char_generated) {
      lsvg.selectAll("mylabels").data(keys).enter().append("text").attr("class", "legend-text").attr("x", 80).attr("y", function (d, i) {
        return 100 + i * 40;
      }) // 100 is where the first dot appears. 25 is the distance between dots
...
},{"./bar.js":"bar.js"}],"index.js":[function(require,module,exports) {
var d3 = require('d3');
...
slider();
bar();
},{"d3":"../node_modules/d3/index.js","./bar.js":"bar.js","./slider.js":"slider.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
