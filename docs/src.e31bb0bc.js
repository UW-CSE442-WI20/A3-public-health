...
var disease_list = ["Chlamydia trachomatis infection", "Campylobacteriosis", "Salmonellosis", "Human immunodeficiency virus diagnoses", "Invasive pneumococcal disease", "Pertussis", "Shigellosis"];
var color_map = {}; // looping over disease list and map a disease to a color
...
var dated = require('./dated.csv');

var year_month = 0; // in ms
...
module.exports = function (ym) {
  ...
  if (!char_generated) {
    lsvg = d3.select("#bar-legend").append("svg").attr("width", 400).attr("height", 500).append("g");
  } // Parse the Data


  d3.csv(dated).then(function (data) {
    data = data.filter(function (d) {
      var cond = d['ym'] == year_month; // only restric for disease in the disease list
    ...
    console.log(max); // add to look up
    ...
    if (char_generated) {
      d3.select("#x-axis").transition().call(d3.axisBottom(x));
    } else {
      svg.append("g").attr("id", "x-axis").attr("transform", "translate(0," + height + ")").call(d3.axisBottom(x)).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end");
    } // Add Y axis


    var y = d3.scaleLog().domain([1, max]).range([height, 0]).base(5);

    if (char_generated) {
      d3.select("#y-axis").transition().call(d3.axisLeft(y));
    } else {
      svg.append("g").attr("id", "y-axis").call(d3.axisLeft(y));
    } // remove ticks without a number


    document.querySelectorAll(".tick").forEach(function (e) {
    ...
    if (char_generated) {
      d3.selectAll('rect').data(data).transition().attr("x", function (d) {
        return x(d.Disease);
      }).attr("y", function (d) {
        return y(d.Count);
      ...
      .attr("height", function (d) {
        console.log("".concat(d.Disease, ", ").concat(d.Count, ", ").concat(height - y(d.Count)));
      ...
      // create hover window
      var info = d3.select("body").append("div").attr("id", "tooltip");
      svg.selectAll("mybar").data(data).enter().append("rect").attr("disease", function (d) {
        return d.Disease;
      ...
      .attr("height", function (d) {
        ...
        return color_map[d.Disease];
      }).on('mouseover', mouseover).on('mouseout', function (d, i) {
        d3.select(this).transition().duration(TRANSITION_TIME).attr("opacity", 1);
        info.transition().duration(TRANSITION_TIME).style("opacity", 0);
        document.querySelectorAll("#bar-legend *[disease='".concat(d.Disease, "']")).forEach(function (e) {
    ...
    function mouseover(d) {
      ...
      d3.select(this).transition().duration(TRANSITION_TIME).attr("opacity", .85); // show hover window

      info.transition().duration(TRANSITION_TIME).style("opacity", 1); // set the content and position the hover window

      info.html("<strong>".concat(d.Disease, "</strong><br>\n                  <strong>Date:</strong> ").concat(document.getElementById("month-year").innerText, "<br>\n                  <strong>Infected Number:</strong> ").concat(parseInt(look_up[d.Disease][year_month]))).style("left", d3.event.pageX + 10 + "px").style("top", d3.event.pageY - 15 + "px").style("position", "absolute");
      document.getElementById("tooltip").className = d.Disease; // highlight legend
    ...
    var size = 20; // Add one dot in the legend for each name.

    if (!char_generated) {
      lsvg.selectAll("mydots").data(keys).enter().append("circle").attr("disease", function (d) {
        return d;
      }).attr("class", "legend-circles").attr("cx", 60).attr("cy", function (d, i) {
        return 100 + i * 40;
    ...
    if (!char_generated) {
      lsvg.selectAll("mylabels").data(keys).enter().append("text").attr("disease", function (d) {
        return d;
      }).attr("class", "legend-text").attr("x", 80).attr("y", function (d, i) {
        return 100 + i * 40;
      }) // 100 is where the first dot appears. 25 is the distance between dots
...
},{"./dated.csv":"dated.csv"}],"slider.js":[function(require,module,exports) {
module.exports = function () {
  var bar = require("./bar.js");

  var MONTHS = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  var YERAS = ["2016", "2017", "2018"]; // add the label
  ...
  function update() {
    ...
    var year = Math.floor(sliderValue / 12);
    document.getElementById("month-year").innerText = "".concat(MONTHS[month], " ").concat(YERAS[year]); // update bar chart

    bar(this.value);
  }

  slider.addEventListener("input", update);
  slider.onclick = update;
};
},{"./bar.js":"bar.js"}],"play.js":[function(require,module,exports) {
...
var playBtn = require("./play.js");

var DEBUG_MODE = true; // turn off console.log when not in debug mode
...
playBtn();
},{"d3":"../node_modules/d3/index.js","./bar.js":"bar.js","./slider.js":"slider.js","./play.js":"play.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
