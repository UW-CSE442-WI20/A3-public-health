...
var myUtilityFunc = require('./utility-funcs');
...
setInterval(function () {
  var lastDataPoint = generateData();
  myData.push(lastDataPoint); // Remove the first element if there are more than 10.

  if (myData.length > 10) {
    myData.shift();
  }

  d3.selectAll('circle').transition().attr('r', rScale(lastDataPoint));
  drawMyData();
}, 1000); // console.log(myData)
...
var rScale = d3.scaleSqrt().domain([0, 1]).range([10, 100]); // A scale is just a function that takes in a value in the
...
var drawMyData = function drawMyData() {
  ...
  // in practice.
  var updateSet = d3.selectAll('div.dataPoint').data(myData, function (d) {
    return d;
  }); // The enter set (any new data point).

  updateSet.enter().append('div').attr('class', 'dataPoint').text(function (d) {
    return d;
  ...
  updateSet.style('color', 'black'); // Any data point that disappeared.

  updateSet.exit().remove();
};

drawMyData();
},{"d3":"../node_modules/d3/index.js","./utility-funcs":"utility-funcs.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
